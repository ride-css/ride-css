...
calcFloat = function (column) {
  var _gutter = this.calcGutter()
    , _padding = this.calcPadding()
    , _width = this.calcColumn(column)

  return _width + (_gutter /2) + _padding;
}
...
module.exports = function () {
  return function (style) {
    style.define('getGrid', function (width, columns, gutter, padding) {
      var mygrid = new GridWidth(width.clone().val, columns.clone().val, gutter.clone().val.split(' '), padding.clone().val.split(' '));
      ...
      return utils.coerceObject({
        ...
        padding: mygrid.padding,
        subtractGutter: mygrid.calcGutter(),
        subtractPadding: mygrid.calcGutter()
      }, {});
    });
